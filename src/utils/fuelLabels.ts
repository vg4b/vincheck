// Friendly labels for the registry `Palivo` (fuel) field.
//
...
/** Friendly base label only (no raw value), or null if unknown/placeholder. */
export function fuelBaseLabel(raw: string): string | null {
	if (!raw) return null
	const norm = normalize(raw)
	if (isPlaceholder(norm)) return null
	return FUEL_LABELS[norm] ?? classifyFuel(norm)
}
